...
If the box is potentially is a default configured IIS server, I'd like to directory crawl it to see if there are any misconfigured goodies potentially available. ```gobuster``` is generally my preferred tool for something like this.
...
As I kind of suspected, we don't find much of anything excited a directory structure that returned ```HTTP 301```, which is forbidden, anyways.
